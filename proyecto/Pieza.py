...
class Pieza:
        ...
        for columna in range(8):
            ...
            for fila in range(8):
                ...
                if abs((columna - self.posicion[0])) == abs((fila - self.posicion[1])) and (
                    ...
                    lista_de_movimientos.append((columna, fila))
        return lista_de_movimientos
    def posibles_movimientos_de_rey(self):
        lista_de_movimientos=[]

        lista_de_movimientos.append((self.posicion[0]+1,self.posicion[1]))
        lista_de_movimientos.append((self.posicion[0]+1, self.posicion[1]+1))
        lista_de_movimientos.append((self.posicion[0]+1, self.posicion[1]-1))
        lista_de_movimientos.append((self.posicion[0]-1, self.posicion[1]))
        lista_de_movimientos.append((self.posicion[0]-1, self.posicion[1]+1))
        lista_de_movimientos.append((self.posicion[0]-1, self.posicion[1]-1))
    ...
    def posibles_movimientos(self):
        ...
        if TipoPieza.REINA==self.tipo:
            return self.posibles_movimientos_de_reina()
        if TipoPieza.REY==self.tipo:
            return self.posibles_movimientos_de_rey()
